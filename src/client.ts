...
export async function getAtolToken(): Promise<string> {
  ...
  const data = await response.json() as { token?: string; error?: { code: number; text: string } };
  if (data.error) {
...
export async function atolPost(path: string, body: unknown): Promise<unknown> {
...
export async function atolGet(path: string): Promise<unknown> {
  const token = await getAtolToken();
  const groupCode = getGroupCode();
